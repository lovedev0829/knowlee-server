...
export async function youtubeAuthCallbackController(
  req: Request,
  res: Response
) {
  try {
    ...
    const tokens = await ({ code: code as string });
    ...
    for (const key in tokens) {
      ...
      updateObject[`youtube.token.${key}`] = value;
    }

    // update youtube token in database
...
export async function youtubeLoginController(req: Request, res: Response) {
  const user = req.user;
