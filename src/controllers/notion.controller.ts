...
export async function notionAuthCallbackController(
  ...
  try {
    ...
    const tokens = await notionAcquireTokenByCode({ code: code as string });
    ...
    for (const key in tokens) {
      ...
      updateObject[`notion.token.${key}`] = value;
    }

    // update notion token in database
...
export async function notionLoginController(req: Request, res: Response) {
  const user = req.user;
  if (!user) throw new RequestError("Could not verify user", 401);
  const { id: userId } = user;

  const authURL = await notionGetAuthCodeUrl({ userId });
